...
CHAMFER = 1.0
CHAMFER_INNER = (
    CHAMFER * sqrt(2) / 2 + WALL_THICKNESS - WALL_THICKNESS * sqrt(2)
) * sqrt(
    2
)  # Chamfer to ensure min wall thickness in corners
FILLET_INNER_Z_RADIUS = (TOTAL_HEIGHT - CUTOUT_HEIGHT - WALL_THICKNESS - 1) / 2
HOLE_CHAMFER = 0.6

NAME_SIZE = (
    TOTAL_HEIGHT * 0.6
)  # Default name size. If the name is too long, the size will be reduced to fit.
NAME_MARGIN = 0.4
...
def find_colinear_edges(part, reference_edges, tolerance=0.01):
    ...
    for ref_edge in reference_edges:
        ...
        for edge in part.edges():
            edge_pos = edge @ 0.5
            if (
                abs(edge_pos.Y - ref_pos.Y) < tolerance
                and abs(edge_pos.Z - ref_pos.Z) < tolerance
            ):
...
with BuildPart() as whistle:
    ...
    with BuildSketch():
        ...
        # Cutout for teeth
        with Locations(
            [(mouth_x - TOOTH_OFFSET, h2), (mouth_x - TOOTH_OFFSET, -h2 + TOOTH_HEIGHT)]
        ):
            Rectangle(
                TOOTH_LENGTH,
                TOOTH_HEIGHT,
    ...
    with BuildSketch(Plane.XY.offset(WALL_THICKNESS)):
        ...
        with Locations((inner_x, 0)):
            Circle(h3)
            Rectangle(cutout_x - inner_x, 2 * h3, align=(Align.MIN, Align.CENTER))
    ...
    tunnel_start_face = (
        whistle.faces()
        .filter_by(Plane.YZ.offset(cutout_x - TUNNEL_CUTOUT_LENGTH))
        .sort_by(Axis.X)[1]
    )
    ...
    inner_edges = [
        ...
        < (e @ 0.5).Z
        < TOTAL_WIDTH - WALL_THICKNESS + EPSILON
    ]
    ...
    inner_edges_z = filter_edges_by_position(
        inner_edges_z, x=cutout_x, y=-h2 + WALL_THICKNESS
    )
    ...
    if name:
        ...
        while font_size > 1:
            temp_text = Compound.make_text(
                name, font_size=font_size, font=font, font_style=font_style
            )
            ...
            text_max_x = (
                cutout_x - CUTOUT_LENGTH + CUTOUT_LENGTH * margin / CUTOUT_HEIGHT
            )
